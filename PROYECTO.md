...
- **Fotos con logo del proveedor:** algunas fotos del catálogo de Nestor Parfum pueden traer su propio logo/marca de agua encima, lo cual el usuario no quiere mostrar (revela el proveedor). No hay forma automática de detectarlo todavía (se revisó una muestra de 7 fotos de marcas distintas y ninguna tenía logo visible — parece ser un caso minoritario, no la mayoría). Mientras no haya un ejemplo concreto para calibrar un detector, existe `scripts/photo-overrides.json`: una lista manual de `{ matchPattern, action: "hide"|"replace", image? }` que `build-catalog.js` cruza igual que el registro de decants — `"hide"` deja el producto sin foto (fondo de la tarjeta vacío), `"replace"` usa una imagen local que el usuario suministre en `assets/productos/`. **Pendiente:** pedirle al usuario un ejemplo concreto (nombre del producto o captura) para llenar este archivo, y de paso ver si el logo tiene un patrón visual consistente que permita automatizarlo de verdad más adelante.
