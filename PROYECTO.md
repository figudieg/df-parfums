...
- **Modo oscuro/claro (Agosto 2026):** el usuario pidió esto explícitamente porque el fondo blanco/crema "se ve vacío". Hay un botón de sol/luna en el nav (`#themeToggle`) que alterna `data-theme="dark"|"light"` en `<html>`, guardado en `localStorage` (`df-theme`). Sin preferencia guardada, sigue `prefers-color-scheme` del sistema. Toda la paleta está en tokens CSS en `:root` de `styles.css`: `--bg`, `--bg-alt`, `--surface`, `--text`, `--text-soft`, `--heading` (+ `--heading-rgb` para usar en `rgba()`), `--border`, `--border-strong`, `--tint` — estos SÍ cambian de valor entre claro/oscuro. `--green-deep`, `--gold` y `--white` son de marca y casi no cambian (son fondos de botones/pills verdes que se quedan verdes en ambos modos; `--gold` se aclara un poco en oscuro para contraste). **Importante para futuros cambios de estilo:** cualquier color nuevo que se agregue debe usar estos tokens (nunca un hex fijo) para que funcione en ambos modos — si un elemento es un "fondo de página/tarjeta" o "texto", usa los tokens de tema; si es literalmente "un botón verde de marca", usa `--green-deep`/`--white` fijos.
