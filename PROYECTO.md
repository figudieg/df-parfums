...
scripts/
  ...
  catalog-missing-photos.json → se regenera solo en cada build: lista de productos sin foto (id + nombre) a la espera de que se agregue una propia en assets/images/products/
assets/images/products/    → fotos propias del usuario para productos sin foto limpia del proveedor (ver sección de Fotos)
...
- **Logo del proveedor en las fotos — RESUELTO (Agosto 2026) con detección por nombre de archivo, sin OCR.** Historial de intentos, para no repetir lo que ya no funcionó:
  1. **OCR sobre el eslogan ("EL PERFUME QUE MARCA TU ESENCIA")** — descartado, seguía dejando pasar casos (Guess Marciano, Paris Hilton Gold Rush, Perry Ellis 360, Creed Aventus, Club de Nuit Sillage, todos con el logo clarísimo pero sin detectar).
  2. **Recorte + tapado geométrico universal en CSS** (aplicado a TODAS las fotos): tapaba el logo 100% confiable, pero **el usuario lo rechazó dos veces** (primero un recuadro blanco sólido, después un degradado tipo viñeta) — dijo que se veía poco profesional y que afectaba también a las fotos limpias, que son la mayoría. Descartado por completo, ya no queda nada de este código.
  3. **★ Solución actual: detección por el nombre del archivo, sin bajar ni procesar la imagen.** El nombre del archivo en el CDN del proveedor ya indica si trae su logo pegado: las fotos con logo tienen un **guión bajo** antes de `nestor_parfum` en la URL (`_nestor_parfum` o `__nestor_parfum`); las limpias (render de fábrica) tienen **guión normal** (`-nestor_parfum`). Confirmado visualmente uno por uno contra 6 fotos con logo y 9 limpias, cero excepciones. De las ~1600 fotos del catálogo, ~303 (≈19%) traen el guión bajo.
     - En `build-catalog.js`: `const hasProviderLogo = /_nestor_parfum/.test(item.image || '');` — si es `true`, `siteItem.image` queda `''` (igual que un producto "agotado": no se muestra ninguna foto, ni recortada ni tapada).
     - En `script.js`/`styles.css`: las tarjetas SIN foto (ya sea por esto o por estar "agotado") muestran un ícono de frasco de perfume prolijo (`.mini-card-photo-placeholder`, SVG de líneas) centrado en vez de un espacio vacío o un parche. Las tarjetas CON foto la muestran completa, sin ningún recorte (`object-fit: cover` normal, sin offsets) — ya no hace falta recortar nada porque las que se muestran ya están garantizadas limpias.
     - Esto es determinístico (no probabilístico como el OCR), no necesita descargar la imagen ni `sharp`/`tesseract.js`, y protege automáticamente productos nuevos sin revisar.
  - Si el proveedor cambiara su convención de nombres de archivo, hay que revisar el regex `/_nestor_parfum/` en `build-catalog.js`.
- **Fotos de "decant" que no dicen "decant" en el título (Agosto 2026, mismo pedido "super estricto" del usuario).** El proveedor vende algunos perfumes en tamaño decant (ej. 10 ML) con una foto que trae un dibujo grande "DECANT" + pastilla "N ML" — pero el título de texto que scrapeamos casi nunca dice "decant" (ej. `"CACHAREL ANAIS ANAIS • DAMA • 10 ML"`, o peor, `"PARIS HILTON CAN CAN EDP • DAMA • 100 ML (NUEVA PRESENTACIÓN)"` con precio de decant pero tamaño mal etiquetado como 100 ML). El filtro viejo (`exclude-decant` en `classify()`, que busca la palabra "DECANT" en el título) no agarra estos casos porque el texto simplemente no lo dice. Se agregó `scripts/detect-decant-photos.js` (reemplaza a `detect-watermarks.js`, mismo lugar en `npm run sync-catalog`): hace OCR sobre el 40% superior de cada foto buscando la palabra "DECANT" (mucho más fácil de leer que la marca de agua — es texto grande, negro, buen contraste) y marca `item.decantPhoto = true`. `build-catalog.js` fuerza el bucket a `exclude-decant` para esos productos sin importar lo que diga el título.

**Marcas que faltaban en las listas de curación (Agosto 2026).** El usuario notó que "Stallion Uomo" no aparecía en el sitio aunque sí está en el proveedor: el título no dice "EMPER" (a diferencia de sus otras variantes, ej. "EMPER STALLION 53"), así que `classify()` no lo reconocía y quedaba `unclassified`. Se agregó `STALLION` a `ARABE_BRANDS`. De paso se revisó el resto de `scripts/catalog-unclassified.json` y se agregaron otras marcas reales que faltaban a `DESIGNER_BRANDS`: `CABOTINE`/`GRES`, `THIERRY MUGLER`/`MUGLER`, `JENNIFER LOPEZ`/`JENNIFER LÓPEZ`, `HOLLISTER`, y el typo `PHILLIPP PLEIN` (con doble L, así viene escrito en el proveedor) además del `PHILIPP PLEIN` correcto que ya estaba. **Si el usuario reporta otro perfume "que no aparece", el primer paso siempre es buscarlo en `scripts/catalog-unclassified.json` (o en `scripts/nestor-parfum-raw.json` si ni siquiera está ahí) antes de asumir que es un problema de sincronización** — la mayoría de las veces es una marca real que le falta a alguna de las listas en `build-catalog.js`, no un bug. **Ojo con el caso contrario:** si el producto SÍ está en `nestor-parfum-raw.json` pero NO aparece ni en `catalog-unclassified.json`, revisar `EXCLUDE_BRANDS` — puede estar excluido a propósito. Eso le pasó a Dumont Paris (línea "Nitro"): estaba en `EXCLUDE_BRANDS` junto con clones descartados aposta (Cuba, Mast Perfume, New Brand, etc.), pero el usuario pidió reincluirla (31-ago-2026) porque sí la quiere vender. Se movió de `EXCLUDE_BRANDS` a `ARABE_BRANDS` (mismo estilo de casa "similar a X" que Armaf/Lattafa/Bharara, que ya viven en esa categoría). **Antes de sacar una marca de `EXCLUDE_BRANDS` o de cualquier lista de curación, confirmar con el usuario** — fueron decisiones de curación deliberadas, no bugs, y solo él sabe cuáles cambiaron de opinión.

Aprovechando esa pregunta, el usuario pidió revisar el resto de `EXCLUDE_BRANDS` (mismo día). Salieron 3 más: `MAST PERFUME` (2 de sus 3 productos ni usan su propio nombre en el título, dicen "Bharara") y `MACARENA` (tiene línea propia "King Of Kings", no son solo copias) pasaron a `ARABE_BRANDS`; `SHAKIRA` y `BENETTON` pasaron a `DESIGNER_BRANDS` porque son marcas propias reales (celebridad / casa de moda), no clones — la exclusión original las había metido en la misma bolsa que los clones sin ser lo mismo. Quedan excluidos a propósito: `NEW BRAND`, `CUBA`, `FRAGLUXE`/`FRAGLUX` (puros clones "similar al X", sin línea propia), `GRANDEUR TUBBEES` (brumas corporales "gourmand" tipo postre, no es perfume tradicional) y `ADIDAS` (sin stock actual del proveedor). **La clasificación por marca es automática**: no hace falta re-hacer nada de esto cuando el proveedor agregue productos nuevos de estas marcas — `classify()` los agarra solos en cada sync. La detección de logo del proveedor (`hasProviderLogo`) también es automática e independiente de la marca (mira el nombre del archivo de la foto, no el texto del producto).

**Categoría nueva "Económico" (31-ago-2026).** El usuario pidió mostrar también `NEW BRAND`, `CUBA`, `FRAGLUXE`/`FRAGLUX` y `GRANDEUR TUBBEES` (clones genéricos sin casa de origen reconocida, a diferencia de Árabes que sí son casas reales aunque también clonen) — pero en su propia categoría, para dejar claro que son económicos y no confundirlos con Árabes/Diseñador/Nicho. Se agregó un 5to bucket `economico` en `build-catalog.js` (`ECONOMIC_BRANDS`, agregado a `curatedBuckets`/`bucketOrder`/`bucketLabel`) y su botón de filtro `data-cat="economico"` en `index.html`. `ADIDAS` se mantuvo en `EXCLUDE_BRANDS` (0 productos en stock del proveeedor a la fecha, no afecta nada). Verificado en el navegador: la ocultación de fotos con logo del proveedor (`hasProviderLogo`) sigue aplicando igual en esta categoría (34 de 89 productos económicos no muestran foto por traer el logo, el resto sí se ve con foto limpia) — es el mismo mecanismo automático de siempre, no hizo falta tocarlo.

**Fotos propias para productos sin foto limpia (8-sep-2026).** El usuario pidió poder subir fotos propias para los productos que quedan sin imagen (logo del proveedor detectado, o el proveedor nunca subió foto) — a la fecha son 212 de 971 (156 Diseñador, 30 Económico, 17 Árabes, 6 Nicho, 3 Estuche; el desglose exacto y actualizado siempre está en `scripts/catalog-missing-photos.json`, se regenera solo en cada build). Se reemplazó el viejo mecanismo `photo-overrides.json` (nunca llegó a usarse — quedó vacío desde que se creó) por uno más simple, sin JSON que mantener a mano: se guarda el archivo de imagen en `assets/images/products/<id>.<ext>`, usando el mismo `id` que ya trae cada producto en `catalogo.js` (para el `id`/`code` correcto de cada producto pendiente, ver `catalog-missing-photos.json`), y `build-catalog.js` la detecta sola en el próximo build — no hace falta tocar código ni JSON, ni siquiera importa la extensión. Si el archivo existe, pisa cualquier otra imagen que hubiera (incluida una foto limpia del proveedor), así que también sirve para reemplazar una por una mejor. Como corre en cada build (incluidas las corridas automáticas cada 12h), la foto propia se re-aplica sola para siempre una vez agregada — no se pierde con el próximo scraping.

Cada tarjeta del grid unificado muestra foto (del proveedor, propia del usuario si se agregó una, o vacía si está agotado o si no hay ninguna de las dos), categoría, nombre, género/tamaño (o "Decant disponible · {tamaño}" si está agotado), nota "similar a X" si aplica (con altura reservada aunque no haya nota, para que todas las tarjetas de una fila midan igual), precio (USD con fondo verde sólido + BCV con chip dorado claro, o las insignias "Agotado"/"Disponible en decant" sin precio si aplica), y botón de WhatsApp — todo generado dinámicamente en `script.js`.
