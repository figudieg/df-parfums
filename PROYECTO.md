...
- **Logo del proveedor en las fotos — EN PROGRESO, esto es lo primero que hay que retomar (Agosto 2026, pedido explícito y estricto del usuario: "super mega importante", cero tolerancia).** Historial de intentos, para no repetir lo que ya no funcionó:
  1. **OCR sobre el eslogan ("EL PERFUME QUE MARCA TU ESENCIA")** — `scripts/detect-watermarks.js` original. Se probó y mejoró dos veces (escala de grises + contraste, exigir una sola palabra clave en vez de dos, revisar también ambas esquinas) y **igual se seguían colando casos** (Guess Marciano, Paris Hilton Gold Rush, Perry Ellis 360, Creed Aventus, Club de Nuit Sillage — todos con el logo clarísimo a simple vista pero `watermark: false`). Descartado: el OCR es probabilístico, nunca da 100%.
  2. **Recorte + tapado geométrico universal en CSS** (aplicado a TODAS las fotos, sin excepción): recortar el ~15% superior + cubrir ambas esquinas inferiores con un rectángulo blanco, y después con un degradado tipo viñeta. **Esto sí tapaba el logo de forma 100% confiable, pero al usuario no le gustó el resultado visual** — dijo que se veía poco profesional (el recuadro blanco se notaba como un parche; el degradado tampoco lo convenció), y señaló correctamente que estaba afectando la presentación de las fotos LIMPIAS también, que son la mayoría. **Este código (`.mini-card-photo img` con `top`/`height` negativos, `.photo-fade`) sigue en `styles.css`/`script.js` en este momento pero hay que reconsiderarlo con el hallazgo de abajo.**
  3. **★ HALLAZGO CLAVE, sin terminar de aplicar todavía:** el nombre del archivo de la foto en el CDN del proveedor **ya indica de forma 100% confiable si trae su logo pegado, sin necesidad de OCR ni de bajar la imagen**. Las fotos con el logo del proveedor tienen un **guión bajo** antes de `nestor_parfum` en la URL (`_nestor_parfum` o `__nestor_parfum`); las fotos limpias (renders de fábrica) tienen un **guión normal** (`-nestor_parfum`, sin underscore). Ejemplos verificados visualmente uno por uno (Agosto 2026): confirmado con **6 fotos que SÍ tienen el logo** (Guess Marciano Dama, Paris Hilton Gold Rush Caballero, Perry Ellis 360 Clásico, Creed Aventus EDP, Armaf Club de Nuit Sillage, Paris Hilton Heiress — todas con `_nestor_parfum`) y **9 fotos confirmadas limpias** (Paco Rabanne Pour Homme, Cabotine Gres, Boum Do Brasil, y 6 variantes de Jo Milano Game of Spades — todas con `-nestor_parfum` sin guión bajo), cero excepciones. Al aplicar este patrón sobre las ~1600 fotos actuales: **303 tienen el guión bajo (traen logo)**, el resto (~1300, ~81%) están limpias.
    - **Esto es muchísimo mejor que el OCR:** es un dato que ya viene en la URL que scrapeamos (`item.image`), no hace falta bajar la foto ni procesarla — es una sola comprobación de texto: `item.image.includes('_nestor_parfum')`.
    - **Lo que falta hacer con esto (la próxima sesión debería empezar por aquí):**
      a. En `build-catalog.js`, agregar `const hasProviderLogo = (item.image || '').includes('_nestor_parfum');` y guardarlo en el `siteItem` (ej. `logoDetected: hasProviderLogo`).
      b. **Repensar el tratamiento visual con este dato ya preciso** (esto es lo que hay que decidir con el usuario, no asumirlo): probablemente lo mejor es mostrar las fotos limpias (~81%) completas y sin ningún recorte/tapado — se ven profesionales, tal como se quejó el usuario que deberían verse — y aplicar el recorte/tapado (o de plano ocultar la foto y mostrar un ícono/placeholder prolijo) **solo** a las ~303 que sí lo tienen. Como ya no depende de un OCR falible, esto sí se puede hacer con confianza total. Falta decidir con el usuario cuál tratamiento prefiere para esas 303 (¿recorte+tapado solo en esas?, ¿ocultar la foto directo?) — mostrarle boceto/ejemplo antes de aplicarlo a todo el catálogo otra vez, ya van dos vueltas que no le gusta el resultado visual.
      c. Si se decide seguir recortando/tapando (aunque sea solo para las 303), probablemente valga la pena una vez más ajustar el estilo visual conversando con el usuario primero — pidió explícitamente "algo super profesional" y ya rechazó dos propuestas (recuadro blanco sólido, degradado tipo viñeta).
      d. Ya no hace falta correr OCR para esto en absoluto — `scripts/detect-watermarks.js` (el original) puede borrarse del todo una vez se migre a este método; ni siquiera hace falta el paso de descarga+`sharp`+`tesseract.js` para el logo (sigue haciendo falta para decants, ver abajo).
  - `scripts/photo-overrides.json` sigue disponible para casos puntuales: `"hide"` para ocultar una foto específica a mano, `"replace"` para poner una foto propia del usuario en `assets/productos/` en vez de la del proveedor.
...
0. **★ EMPEZAR POR AQUÍ: aplicar el hallazgo del nombre de archivo (`_nestor_parfum`) para el logo del proveedor.** Ver la sección de arriba "Logo del proveedor en las fotos" — ya está validado y confirmado visualmente, solo falta implementarlo en `build-catalog.js` y decidir con el usuario el tratamiento visual final para las ~303 fotos que sí traen el logo (el recuadro blanco y el degradado tipo viñeta que se probaron NO le gustaron al usuario). El código actual de recorte/degradado universal (en `styles.css`/`script.js`) sigue aplicado a TODAS las fotos por ahora — probablemente haya que quitarlo de las que están confirmadas limpias.
