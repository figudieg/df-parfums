...
- **Logo del proveedor en las fotos — RESUELTO (Agosto 2026) con detección por nombre de archivo, sin OCR.** Historial de intentos, para no repetir lo que ya no funcionó:
  ...
  - `scripts/photo-overrides.json` sigue disponible para casos puntuales: `"hide"` para ocultar una foto específica a mano (por si este patrón de nombre de archivo llegara a fallar en algún caso puntual), `"replace"` para poner una foto propia del usuario en `assets/productos/` en vez de la del proveedor.
