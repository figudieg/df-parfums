...
- **Logo del proveedor en las fotos — resuelto con recorte geométrico, NO con detección (Agosto 2026, pedido explícito y estricto del usuario).** Primer intento: `scripts/detect-watermarks.js` hacía OCR sobre el 10% superior de cada foto buscando el eslogan del proveedor ("EL PERFUME QUE MARCA TU ESENCIA") y ocultaba la foto completa (`image: ''`) si lo encontraba. **Resultó poco confiable en la práctica:** el usuario mandó capturas del sitio en vivo mostrando el logo en varias fotos que el OCR había marcado como limpias (`watermark: false`) a pesar de que el texto era perfectamente legible a simple vista — se probó mejorar el OCR (escala de grises + contraste, exigir una palabra clave en vez de dos, revisar también las esquinas inferiores) y aun así se seguían colando casos (ej. "Guess Marciano EDP" y "Paris Hilton Gold Rush" con la variante de cinta diagonal). El usuario fue explícito: esto es "super mega importante" porque no es su marca y puede traer problemas — cero tolerancia a que se cuele una.
  - **Solución actual: recorte y tapado geométrico en CSS, aplicado a TODAS las fotos, siempre, sin depender de ninguna detección.** En `styles.css`, `.mini-card-photo img` se posiciona con `top:-18%` / `height:118%` / `object-fit:cover` dentro de un contenedor con `overflow:hidden`, lo que recorta el ~15% superior de cada foto (ahí vive el eslogan). Dos `<span class="photo-mask photo-mask-bl/br">` (42% ancho × 34% alto, fondo blanco, `z-index:2`) tapan ambas esquinas inferiores por encima de la imagen, cubriendo tanto la variante de insignia chica como la cinta diagonal grande, sea cual sea el producto. Esto es determinístico (no probabilístico como el OCR) y protege automáticamente productos nuevos que nunca se hayan revisado, que es justo lo que pidió el usuario.
  - Como consecuencia, `build-catalog.js` ya **no oculta ninguna foto por marca de agua** — el campo `image` del proveedor se usa siempre tal cual, el recorte lo hace el CSS del lado del sitio. El campo `watermark` en `nestor-parfum-raw.json` quedó sin uso (no se lee en ningún lado); se puede ignorar o limpiar en el futuro.
  - Si el proveedor cambiara el tamaño/posición de su marca de agua de forma que ya no quepa en estos porcentajes, hay que ajustar los números en `.mini-card-photo img` / `.photo-mask` en `styles.css` — no hay ningún script que tocar.
