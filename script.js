...
// ---------- Catalogo unico (destacados con decant + inventario del proveedor) ----------
const WA_NUMBER = '584121985211';
const allProducts = [...(window.DESTACADOS_DECANT || []), ...(window.CATALOGO_NESTOR || [])];
let catFilter = 'all';
let catSearch = '';
let catVisible = 30;
const CAT_BATCH = 30;
...
function renderCard(item) {
  return `
    <div class="mini-card${item.decant ? ' mini-card-decant' : ''}" ${item.decant ? `data-decant-id="${item.id}"` : ''}>
...
// ---------- Modal de detalle (solo para los productos con decant) ----------
...
function openModal(item) {
  ...
  modalPriceUsd.textContent = item.priceUsd != null ? fmtPrice(item.priceUsd) + ' Divisas/Efectivo' : '';
  modalPriceBcv.textContent = item.priceBcv != null ? fmtPrice(item.priceBcv) + ' BCV' : '';
  modalPriceBcv.style.display = item.priceBcv != null ? '' : 'none';
  modalDecant.style.display = item.decant ? '' : 'none';
  modalWhatsapp.href = buildCatWaLink(item);
...
if (allProducts.length) {
  ...
  if (miniGrid) {
    miniGrid.addEventListener('click', (e) => {
      if (e.target.closest('.mini-card-wa')) return;
      const card = e.target.closest('.mini-card[data-decant-id]');
      if (!card) return;
      const item = allProducts.find((p) => p.id === card.dataset.decantId);
